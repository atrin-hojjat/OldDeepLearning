...
Learn::Learn(MatrixXd input, MatrixXd output,double learning_rate,double regularization_parameter)
        : regularization_parameter(regularization_parameter),learning_rate(learning_rate),input(input),output(output)
{
    learn();
    drawLearningPlot();
}

Learn::Learn(MatrixXd input, MatrixXd output,double learning_rate,double regularization_parameter,long max_iters)
        : regularization_parameter(regularization_parameter),learning_rate(learning_rate),
          input(input),output(output),max_iters(max_iters)
{
    learn();
    drawLearningPlot();
}
...
//    cout << endl << input << endl<<endl;
    ...
    _iter_stats.insert(_iter_stats.end(),J(w));

    for(;;){
        wpast = w;

        for (int i = 0; i < input.cols(); ++i) {
            VectorXd err = (output.col(i) - w*input.col(i));
            double squared_err = err.transpose()*err;
            w = w + 2 * learning_rate * err * input.col(i).transpose();
            _iter_stats.insert(_iter_stats.end(),J(w));
        }
...
void Learn::drawLearningPlot() {
    ...
    for (int i = 0; i < numPoints; ++i) {
        table->SetValue(i, 0, i*input.cols());
    ...
    vtkSmartPointer<vtkFloatArray> arrX2 =
            vtkSmartPointer<vtkFloatArray>::New();
    arrX2->SetName("Number Of Iterations");
    table2->AddColumn(arrX2);

    vtkSmartPointer<vtkFloatArray> arrC2 =
            vtkSmartPointer<vtkFloatArray>::New();
    arrC2->SetName("Cost Function - For Every Change");
    table2->AddColumn(arrC2);

    // Fill in the table with some example values
    int numPoints2 = _iter_stats.size();
    table2->SetNumberOfRows(numPoints2);
    for (int i = 0; i < numPoints2; ++i) {
        table2->SetValue(i, 0, i);
        table2->SetValue(i, 1, _iter_stats[i]);
    }
